...
(function () {
    ...
    function drainToPriority(priority, priorityName) {
        output("<br/>Scheduling Jobs...");

        // schedule some work
        S.schedule(function () {
            output("Running job1 at normal priority");
        }, S.Priority.normal);


        S.schedule(function () {
            output("Running job2 at high priority");
        }, S.Priority.high);


        S.schedule(function () {
            output("Running job3 at idle priority");
        }, S.Priority.idle);


        S.schedule(function () {
            output("Running job4 at belowNormal priority");
        }, S.Priority.belowNormal);


        S.schedule(function () {
            output("Running job5 at normal priority");
        }, S.Priority.normal);



        output("Draining scheduler to " + priorityName + " priority");
        S.requestDrain(priority).done(function () {
            output("Done draining");
        });
    }
